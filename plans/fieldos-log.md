...
## 2026-08-19 — Alpha shipped, and a first-run experience started

**Alpha is complete and tagged `v0.1.0-alpha.1` (`460ab56`).** All 20 Alpha issues Done, no open
work in the milestone, and the release build verified reproducible on a clean CI machine for the
first time — 9 workers, 30 modules, 155 asset blobs, two builds producing an identical manifest.

**OZL-237 was mostly already done, and the real bug was elsewhere.** The ticket asked for
"semantic versioning, unit testing, integration testing, qa/regression, packaging"; OZL-242/253/256
had delivered all but the first. What was genuinely missing was versioning (`fieldos@0.1.0-alpha.1`,
replacing `cloudflare-os@1.0.0`) and a way to see that a release is what it claims to be.

The reproducibility check written for that earned its place immediately: **it found that the
release build could never work on a clean checkout.** `src/generated/app.txt` and
`format-blueprints.ts` are gitignored and generated by each package's own `build`, which
`wrangler deploy --dry-run` bypasses. Every prior success depended on artifacts left by an earlier
run. Fixed by running each package's own build before bundling it — after a first attempt that
named two specific generators and was still wrong when CI hit the next one.

**Five hypotheses died on the way to that.** tsconfig `include` scoping, a shared-base fix,
`baseUrl`/`paths` resolution, two producers writing different content (byte-identical, 26810 both),
and a build-parallelism race (parallel from clean: 0 errors). Each was killed by executing it. The
wrong ones are recorded because they are cheap to re-derive.

**OZL-222 answered itself once the code was read.** The handoff said an org-less user is "locked
out"; they are not. Enforcement lives in `open()`'s non-owner branch, so owners always reach their
own workspaces and only *collaboration* stops. And the policy question was already settled at
`plans/org-separation.md:61` — "a missing claim means no org, never a default org" — for a reason
worth keeping: above 200 groups Entra omits the groups claim entirely and substitutes a Graph
pointer unreachable from an airgapped network, so a default org would convert an authentication
failure into a silent authorization grant. I had drafted "org-less users get a default org" as a
candidate before reading that. The gap was never the org model; it was that the doc's own
mitigation — showing the operator "no org resolved" — had never been built.

**A near-miss worth recording.** `org-separation.md:221` asks for a "distinct error, not a generic
denial", and I started making the *user-facing* denial distinguishable. `api.ts:290` forbids
exactly that, in a comment ending "Do not 'clarify' this string": an identical message is what
stops the error leaking that another org holds a workspace at that id. The read-out is for the
operator; the denial stays generic. Both are now stated in the design doc.

**Local deployment is real now, and three things had to be fixed to get there.** The stack runs on
standalone workerd with `guy` as admin. `run-workerd.mjs` now loads `.env.local` (stdlib
`process.loadEnvFile`, no dependency) and forwards per-gatekeeper OAuth credentials, neither of
which had any path before — `gatekeeper-github` was bound but could only ever answer "not
configured".

**The frontend silently reverted to the Vite dev port three times.** `VITE_BACKEND_HOST` is baked
in at build time, so any plain `pnpm build` rebuilds it for 8787 and every RPC fails. `run-workerd`
detects this and *warns*; the warning was read past each time, including by me while declaring the
deploy healthy. Loading `.env.local` closes it, but the general lesson is that a warning on a path
someone runs dozens of times is not a control.

**OZL-311 started: a guided first build, not a tooltip tour.** Seeded context now installs itself
on an admin's first visit to the Context Library. That trigger is the load-bearing decision: a
public collection requires admin, `isAdmin` arrives only through `startAppUi({ isAdmin })`, and a
backend-callable seeding path was considered and rejected because it would let a gatekeeper create
domain-wide content with authority no human granted. The human admin *is* the authority. Cost:
a deployment nobody administers gets no seeded content — a visible absence, where the alternative
would be an invisible one.

Stopped before the walkthrough component deliberately, with a third of a context window left,
rather than leave an unfinished `workshop-shared` API change for a reviewer to untangle.

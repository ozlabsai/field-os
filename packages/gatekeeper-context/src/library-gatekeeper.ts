...
import { installSeedCollections } from "./seed-collections.js";
...
export class ContextAccount
  ...
  async startAppUi(context: AppUiContext): Promise<GatekeeperUiFrame> {
    // Hand the iframe its per-user UI capability. isAdmin and orgId are supplied fresh per open;
    // neither may be read from props, which are fixed at provisioning and go stale permanently.
    let api = new ContextApiImpl(
      this.env, this.ctx.props.sharingDomain, this.ctx.props.accountId, context.isAdmin,
      context.orgId, this.#collections(), this.#userLibraries(), this.#registries());

    // Install the bundled sample collections, if this domain has none yet and the caller is an
    // admin. Here rather than at deploy because a public collection needs admin authority and
    // this is the only place it arrives -- see seed-collections.ts for why a backend-callable
    // path was rejected. Awaited so the library the admin is about to see already contains them;
    // it is a handful of KV writes on the rare visit where anything is missing, and a no-op
    // afterwards.
    await installSeedCollections(
      api,
      await listPublicCollectionsFromKv(this.env, this.ctx.props.sharingDomain),
      context.isAdmin);

    let ui = new RpcStub(api);
...
export class ContextGatekeeper
  ...
  async getSlashCommandProvider(context?: SessionContext):
      Promise<ContextSlashCommandProvider> {
    return new ContextSlashCommandProvider(
        () => this.#listSlashCommands(context?.orgId),
        (id, args, authorizer) => this.#invokeAgentSkillCommand(id, args, authorizer, context));
  }

  async #listSlashCommands(readerOrg: string | undefined): Promise<SlashCommandDescriptor[]> {
    let domain = this.ctx.props.sharingDomain;
    let userLibrary = this.#userLibraries().get(
        this.#userLibraries().idFromName(domainName(domain, this.ctx.props.accountId)));
    let collections = (await loadEnabledContextCollections(
            this.env, domain, userLibrary, readerOrg, isOrgSeparationEnabled(this.env)))
        .toSorted((left, right) =>
  ...
  async #invokeAgentSkillCommand(
    ...
    // The session carries the org too, so invoking a command by a guessed id cannot reach a
    // cross-org skill that `#listSlashCommands` declined to show.
    using session = this.#newReadSession(authorizer, context);
  ...
  async getAgentCatalog(
      ...
      authorizer: NativeRpcStub<ObservationAuthorizer>,
      context?: SessionContext): Promise<AgentCatalog> {
    let domain = this.ctx.props.sharingDomain;
    let userLibrary = this.#userLibraries().get(
      this.#userLibraries().idFromName(domainName(domain, this.ctx.props.accountId)));
    let collections = await loadEnabledContextCollections(
        this.env, domain, userLibrary, context?.orgId, isOrgSeparationEnabled(this.env));
